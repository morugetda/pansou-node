...
export class DownloadHelper {
    ...
    async getDirectLink(originalUrl, type) {
        try {
    ...
    async parseTianyiLink(url) {
        return { url, type: 'redirect' };
    }
    async parse115Link(url) {
        return { url, type: 'redirect' };
    }
    async parseUCLink(url) {
        return { url, type: 'redirect' };
    }
    async parseXunleiLink(url) {
        return { url, type: 'redirect' };
    }
    extractBaiduShareId(url) {
        const match = url.match(/\/s\/([a-zA-Z0-9_-]+)/);
        return match ? match[1] : null;
    }
    extractAliyunShareId(url) {
        const match = url.match(/\/s\/([a-zA-Z0-9]+)/);
        return match ? match[1] : null;
    }
    extractQuarkShareId(url) {
        const match = url.match(/\/s\/([a-zA-Z0-9]+)/);
        return match ? match[1] : null;
    }
...
downloadAll();
    ...
    // 生成 aria2c 下载命令
    generateAria2Commands(links, password, options) {
        const commands = [];
    ...
    // 生成 Aria2 RPC 配置
    generateAria2RPCConfig(links, config) {
...
echo "已添加 ${links.length} 个下载任务到 Aria2"
    ...
    generateQBTCommands(links, qbUrl, username, password) {
...
echo "已添加 ${links.length} 个下载任务到 qBittorrent"
    ...
    generateTransmissionCommands(links, trUrl, username, password) {
...
echo "已开始 ${links.length} 个迅雷下载任务"
    ...
    // 生成 Python 下载脚本
    generatePythonDownloadScript(links, options) {
        return `
#!/usr/bin/env python3
# Python 批量下载脚本
...
if __name__ == "__main__":
    ...
    // 生成 IDM 下载列表
    generateIDMList(links, password) {
        let list = '';
        links.forEach((link, index) => {
            const filename = link.filename || `file_${index + 1}`;
            list += `${link.url}\n`;
...
export function getDownloadHelper() {
