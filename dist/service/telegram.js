...
export class TelegramSearchService {
    ...
    async search(keyword, options) {
        ...
        for (const batch of batches) {
            const batchResults = await Promise.all(batch.map(channel => this.searchChannel(channel, keyword)));
        ...
        try {
            ...
            for (const apiUrl of endpoints) {
                try {
                    ...
                    const results = response.data.messages.map((msg, index) => {
...
let globalTelegramSearch = null;
export function getTelegramSearch() {
