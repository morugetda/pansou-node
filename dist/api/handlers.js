    ...
    try {
        ...
        if (req.method === 'GET') {
            searchParams = {
                ...
                conc: req.query.conc ? parseInt(req.query.conc) : undefined,
                refresh: req.query.refresh === 'true',
                res: req.query.res,
                src: req.query.src,
                plugins: req.query.plugins ? req.query.plugins.split(',') : undefined,
                cloud_types: req.query.cloud_types ? req.query.cloud_types.split(',') : undefined,
                ext: req.query.ext ? JSON.parse(req.query.ext) : undefined,
        ...
        const result = await searchService.search({
            ...parsed,
            cloud_types: parsed.cloud_types
        });
        let response = result;
...
export async function handleDownloadStatus(req, res) {
    try {
...
export async function handleSyncDownloads(req, res) {
    try {
        const tracker = getDownloadTracker();
...
export async function handleExportTasks(req, res) {
    try {
        const tracker = getDownloadTracker();
...
export async function handleImportTasks(req, res) {
    try {
        const { data } = req.body;
        if (!data) {
            res.status(400).json({
