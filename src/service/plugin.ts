...
abstract class BasePlugin implements Plugin {
  ...
  protected httpClient: ReturnType<typeof getHttpClient>;
  protected config: ReturnType<typeof getConfig>;
  ...
  async search(keyword: string): Promise<PluginResult> {
    try {
      ...
      const fullResults: SearchResult[] = results.map(r => ({
        ...r,
  ...
  protected abstract doSearch(keyword: string): Promise<BaseSearchResult[]>;
...
class PluginManagerClass {
  ...
  async search(keyword: string, pluginNames?: string[]): Promise<PluginResult[]> {
    const plugins = pluginNames 
      ? pluginNames.map(n => this.plugins.get(n)).filter((p): p is Plugin => p !== undefined)
      : this.getPlugins();

    const results = await Promise.all(
      plugins.map(plugin => plugin.search(keyword))
    );
...
class JikepanPlugin extends BasePlugin {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.post(
        'https://api.jikepan.xyz/search',
        { name: keyword, is_all: false },
        {
          timeout: 15000,
          headers: {
  ...
  private convertLinkType(service: string): CloudType | null {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.get(
        `https://www.666pan.com/api/search?q=${encodeURIComponent(keyword)}`
      );
...
class HunhepanPlugin extends BasePlugin {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.get(
        `https://hunhepan.com/api/search?keyword=${encodeURIComponent(keyword)}`,
        {
          headers: {
...
class PansearchPlugin extends BasePlugin {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.get(
        `https://pansearch.org/api/search?q=${encodeURIComponent(keyword)}`
      );
...
class QupansouPlugin extends BasePlugin {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.get(
        `https://www.qupansou.org/search?key=${encodeURIComponent(keyword)}`,
        {
          headers: {
...
class SusuPlugin extends BasePlugin {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.get(
        `https://susu98.com/api/search?q=${encodeURIComponent(keyword)}`
      );
...
class XuexiPlugin extends BasePlugin {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.get(
        `https://www.xuexizhinan.com/api/search?wd=${encodeURIComponent(keyword)}`
      );
...
class Pan789Plugin extends BasePlugin {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.get(
        `https://www.pan789.com/api/search?q=${encodeURIComponent(keyword)}`
      );
...
class WanpanPlugin extends BasePlugin {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.post(
        'https://www.wanpan.com/api/search',
        { keyword },
        { timeout: 10000 }
      );
...
class DuojiPlugin extends BasePlugin {
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    try {
      const response = await this.httpClient.get(
        `https://www.duoji.pw/api/search?q=${encodeURIComponent(keyword)}`
      );
...
let globalPluginManager: PluginManagerClass | null = null;
  ...
  protected async doSearch(keyword: string): Promise<BaseSearchResult[]> {
    ...
    try {
      const response = await this.httpClient.post(
        ...
        { kw: keyword, res: 'merge', refresh: true },
        {
          timeout: 10000,
          headers: {
    ...
    const mockData: BaseSearchResult[] = [];
...
export function getPluginManager(): PluginManagerClass {
